...
header_from_stream_output(const std::string& output)
{
  ...
  iss >> version_token >> det_id_token >> crate_id_token >> slot_id_token >> stream_id_token >> sequence_label >>
    sequence_value >> block_label >> block_length_label >> block_length_value >> timestamp_label >> timestamp_value;
  ...
  return make_header(static_cast<word_t>(std::stoull(version_token.substr(std::string("Version:").size()))),
...
BOOST_AUTO_TEST_CASE(TimestampRoundTripFromGetTimestamp)
{
  const std::vector<word_t> values = {
    0x0000000000000000ULL, 0x0000000000000001ULL, 0x0123456789ABCDEFULL, 0xFEDCBA9876543210ULL, 0xFFFFFFFFFFFFFFFFULL
  };
...
{
  const DAQEthHeader original = make_header(7, 3, 22, 4, 9, 0, 1234, 88, 0x0123456789ABCDEFULL);
...
{
  const DAQEthHeader original = make_header(13, 21, 777, 10, 55, 6, 2048, 1024, 0x1122334455667788ULL);
